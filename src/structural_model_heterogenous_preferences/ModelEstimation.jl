...
# --- Sweep analysis helper -------------------------------------------------
function load_sweep_targets(csv_path::String)
    df = CSV.read(csv_path, DataFrame)
    # Build a mapping from a normalized header name (lowercase, stripped)
    # to the actual column identifier in the DataFrame. This allows headers
    # to be strings or symbols and tolerate minor formatting differences.
    orig_names = names(df)
    norm_map = Dict{String, Any}()
    for n in orig_names
    ...
    required = ["k", "c0", "mean_alpha"]
    if !all(r -> haskey(norm_map, r), required)
        error("sweep CSV must contain columns: k, c0, mean_alpha. Found columns: $(orig_names)")
    end
    # Use the mapped original names to access columns
    k_col = norm_map["k"]; c0_col = norm_map["c0"]; mean_col = norm_map["mean_alpha"]
    k_vals = sort(unique(Float64.(df[!, k_col])))
    c0_vals = sort(unique(Float64.(df[!, c0_col])))

    # Initialize with NaN so missing entries are visible
    mean_mat = fill(NaN, length(c0_vals), length(k_vals))
    var_mat = fill(NaN, length(c0_vals), length(k_vals))

    # Build a lookup from (k,c0) -> values to handle any row ordering or missing cells
    lookup = Dict{Tuple{Float64,Float64}, Tuple{Float64,Float64}}()
    has_var = any(x -> lowercase(String(x)) == "var_alpha", orig_names)
    var_col = has_var ? findfirst(x -> lowercase(String(x)) == "var_alpha", orig_names) : nothing
    for r in eachrow(df)
        kf = Float64(r[k_col]); c0f = Float64(r[c0_col])
        ma = Float64(r[mean_col])
        va = has_var ? Float64(r[var_col]) : NaN
        lookup[(kf, c0f)] = (ma, va)
    end

    for (j, kv) in enumerate(k_vals)
        for (i, c0v) in enumerate(c0_vals)
            key = (kv, c0v)
            if haskey(lookup, key)
                mean_mat[i, j] = lookup[key][1]
                var_mat[i, j] = lookup[key][2]
            else
                mean_mat[i, j] = NaN
                var_mat[i, j] = NaN
            end
    ...
    # Compute elasticities per k using only finite entries
    elasticities = DataFrame(k=Float64[], slope_d_meanalpha_dlogc0=Float64[], elasticity_dlogmeanalpha_dlogc0=Float64[])
    logc_all = log.(c0_vals)
    for (j, kv) in enumerate(k_vals)
        y = mean_mat[:, j]
        # Align lengths defensively in case of inconsistent sweep inputs
        if length(y) != length(logc_all)
            L = min(length(y), length(logc_all))
            y = y[1:L]
            x = logc_all[1:L]
        else
            x = logc_all
        ...
        if count(mask) >= 2
            y_valid = y[mask]
            x_valid = x[mask]
            s = var(x_valid) > 0 ? cov(y_valid, x_valid) / var(x_valid) : 0.0
            avg_mean = mean(y_valid)
            rel = avg_mean > 0 ? s / avg_mean : 0.0
        else
            s = 0.0; rel = 0.0
        end
        push!(elasticities, (kv, s, rel))
    end

    return Dict(:df => df, :k_grid => k_vals, :c0_grid => c0_vals,
        :mean_alpha_matrix => mean_mat, :var_alpha_matrix => var_mat, :elasticities => elasticities)
end
