...
# Include the model setup script locally so the Primitives and Results types
# are available inside this module. ModelSetup.jl is a script (not a submodule),
# so "using .ModelSetup" raises an error when the file isn't declared as a module.
...
function plot_employment_distribution(results, prim)
    ...
    hmap = heatmap!(ax, prim.h_grid, prim.ψ_grid, results.n',
...
function plot_surplus_function(results, prim)
    ...
    ax = create_axis(fig[1, 1],
        ...
        hmap = heatmap!(ax, prim.h_grid, prim.ψ_grid, results.S,
                    colormap = :plasma)
    
    Colorbar(fig[1, 2], hmap, label = "Match Surplus")
    
    # Add a contour plot to show the zero-surplus line
        contour!(ax, prim.h_grid, prim.ψ_grid, results.S,
...
function plot_work_arrangement_regimes(results, prim; gray_nonviable::Bool=false)
    ...
    ψ_min, ψ_max = prim.ψ_min, prim.ψ_max
    ...
    if gray_nonviable
        ...
        heatmap!(
            ax, prim.h_grid, prim.ψ_grid, gray_mask,
            colormap = cgrad([:gray]),
            interpolate = false,
            transparency = true,
            alpha = 1.0
    ...
    xlims!(ax, prim.h_min, prim.h_max)
    ylims!(ax, prim.ψ_min, prim.ψ_max)
    
    # Place the legend outside the plot (to the right). If the backend/version
    # doesn't support `outside=true`, fall back to an in-axis legend.
    try
        axislegend(ax, position = :rb, outside = true)
    catch
...
function plot_alpha_policy_by_firm_type(results, prim)
    ...
    h_indices_to_plot = [
        round(Int, prim.n_h * 0.1),  # A low-skill worker (25th percentile)
        round(Int, prim.n_h * 0.50),  # A medium-skill worker (50th percentile)
        round(Int, prim.n_h * 0.9)   # A high-skill worker (75th percentile)
    ]
